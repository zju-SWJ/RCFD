...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000, memory_efficient=False, is_big_size=False):
        ...
        self.scala = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def forward(self, x, type='both'):
        x = self.features(x)
        x = self.trans1(self.block1(x))
        x = self.trans2(self.block2(x))
        x = self.trans3(self.block3(x))
        x = self.final_norm(self.block4(x))
        x = F.relu(x, inplace=True)
        x = F.adaptive_avg_pool2d(x, (1, 1))

        
        out_feature = torch.flatten(self.scala(x), 1)
        if type == 'feat':
            return out_feature
