...
flags.DEFINE_float('temp', 0.9, help='distillation temperature')
flags.DEFINE_float('alpha', 0, help='alpha') # beta in paper
flags.DEFINE_float('beta', 0, help='beta') # gamma in paper
...
def train():
    ...
    for step in range(1, FLAGS.total_steps + 1):
        ...
        loss, entropy, div, feat = teacher_sampler.module.RCFD(student_sampler, classifier, x_0, y=y,
                                                    temp=FLAGS.temp, alpha=FLAGS.alpha, beta=FLAGS.beta, imagenet_cls=imagenet_cls)
        optim.zero_grad()
        loss.backward()
